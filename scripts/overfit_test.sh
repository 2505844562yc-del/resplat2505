...
CUDA_VISIBLE_DEVICES=0 python -m src.main +experiment=dl3dv     mode=train     dataset.roots=[${DATASET}]     dataset.overfit_to_scene=${SCENE}     dataset.image_shape=[256,448]     dataset.ori_image_shape=[270,480]     dataset.view_sampler.num_context_views=8     dataset.view_sampler.num_target_views=6     trainer.max_steps=${STEPS}     trainer.num_nodes=1     data_loader.train.batch_size=1     model.encoder.num_refine=4     model.encoder.recurrent_use_checkpointing=true     model.encoder.use_amp=true     checkpointing.pretrained_model=pretrained/resplat-base-dl3dv-512x960-view8-8179ed87.pth     checkpointing.no_strict_load=true     optimizer.lr=1e-4     optimizer.lr_monodepth=0.     optimizer.lr_depth=0.     optimizer.warm_up_steps=100     wandb.mode=disabled     train.no_log_video=true     train.no_log_projections=true     output_dir=checkpoints/overfit-test
